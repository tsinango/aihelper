...
- 2026-09-06 Phase 4.3 完成（无新迁移）：worker `pump_once` 单步可测（inbox 永远优先文档任务，无模型时不领 learn）、learn 后未引用块标 `evidence_only`、覆盖率接口与页面（只报有去向比例）、重学重置（删已完成/失败提炼任务行重排，提案指纹去重）、`edit_knowledge` 适用范围/结构变更补历史（修 4.2 遗漏）。回归全绿；生产 E2E 版本覆盖率如实为未完成（pending 块等可用模型），worker 任务带 404 终态可查；验收记录 `data/phase43_acceptance.json`（gitignored）。**仍待人工**：真实 PDF/PPTX 全文验收与可用学习模型二选一决策（见上）。
- 2026-09-06 Phase 5.1 完成（无新迁移，纯代码）：`retrieve_document_evidence()`（合格原文块词法回读）+ 回答流程有界组合（最多 1+1 次生成、整块引用、空读不降级、矛盾记 `knowledge_document_conflict`）、`check_sources` 参数与 Chat 核对按钮、文档引用展示（定位代替 K 前缀）。回归 409+38skip/447；生产验证 run 186（无合格文档时草稿保持 + trace 留痕）与客户端核对流程；rescue 在 pg 证明（生产无合格文档版本，rescue 在生产诚实地未被证明）。验收记录 `data/phase51_acceptance.json`（gitignored）。
- 2026-09-06 Phase 5.2 完成（迁移 025，两库已应用）：版本前驱 + 差异摘要、章节级对比（页码/幻灯片号不算变化）、受影响单元查询（全局范围保守全影响）、只读 impact 与记录式 revalidate（不动旧单元）、请求版本感知的 lineage 门槛、人工验证状态切换（`revalidate` 历史）、快照带版本来源。回归 412+44skip/456；生产 E2E-MANUAL v2 对比与前驱记录如实完成。验收记录 `data/phase52_acceptance.json`（gitignored）。**仍待人工**：两份真实手册版本的对比验收与可用学习模型决策（见 4.2 门）。
- 2026-09-06 Phase 5.3 完成（迁移 026，两库已应用）：失败六分类 + 默认动作（`classify_run_failure`，标注 only）、`GET /api/v2/failures`、Inbox 失败视图与门限进度、`expected_knowledge_ids` 与 10 例门限计数、`evaluate_v2.py --failure-report`。回归 417+47skip/464；生产 30 天 148 例未判定失败如实分类（多为历史未覆盖问题，非回归），门限 0/10 检索保持不动；闭环：R179 服务失败重试解决、R174 纠正经 R177/R190 复测通过、F8 关闭留因。验收记录 `data/phase53_acceptance.json`（gitignored）。**仍待人工**：连续 5 工作日 20+ 真实问题试用、10 例漏召回证据、真实手册与可用学习模型（见前）。
