...
class LearningLoopTest(unittest.TestCase):
    ...
    def test_only_explicit_confirmation_reaches_confirm_handler(self):
        ...
        with patch("v2.learning._confirm") as confirm, patch("v2.learning._model_facts", return_value=([{"title": "修正", "content": "F-NR-208E/2 在旧版不是 1U", "entity_name": "F-NR-208E/2"}], False)), patch("v2.learning._find_knowledge", return_value=None), patch("v2.learning._create_knowledge", return_value={"id": 21}), patch("v2.learning._insert_proposal", return_value={"id": 31}), patch("v2.learning._next_question", return_value=({"id": 41, "content": "我理解为：F-NR-208E/2 在旧版不是 1U。对吗？"}, "", {"id": 31})):
    ...
    def test_confirmation_transition_writes_user_confirmed_and_audit_source(self):
        ...
        self.assertTrue(any("SET trust='user_confirmed'" in query for query in statements))
    ...
    def test_unknown_and_skip_are_legal_without_reasking_same_proposal(self):
        for answer, expected in (("不知道", "unknown"), ("跳过", "skip")):
            ...
            common = self._patch_common(pending)
            with patch("v2.learning._next_question", return_value=(None, None, None)), patch("v2.learning._model_facts") as model:
