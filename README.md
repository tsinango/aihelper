...
续跑，重学幂等、无重复提案。
Phase 5.1 已实现有界原文回读：无合格 Knowledge 时读合格原文（已确认真实
性、解析未失败、版本范围不冲突、非待人工块），原文请求/高风险操作对已有
草稿做一次核对，最多一次初始生成加一次回读生成，整块引用不截断，矛盾只
呈现不自动选边；未核实上传只供人工阅读，不自动引用。
Phase 5.2 已实现文档版本更新与重验：新版只新增不覆盖，章节级对比（加页码
不算变化）定位新增/变化/删除/失配，全局范围变化保守影响全部派生单元；
重验只记录版本关系与差异，旧单元继续服务旧项目，请求明确新版时才排除旧
版链单元；验证状态可由人工切换并审计。
Phase 5.3 已实现失败驱动改进闭环：失败按缺资料/缺知识/召回失败/生成失败/
版本适用/服务失败六类展示各自默认动作，只标注不自动建知识；召回改进门限
10 例真实漏召回（未达则不改检索）；Inbox 可看近期失败与门限进度。
下一阶段：Phase 5 完成，停止。人工试用与真实手册验收见 OPERATIONS 与最终报告。
