...
已有迁移 001–012 是增量迁移，涉及审核候选、发布版本、case memory、Telegram provenance、2048 维向量、review groups 和本地模型评测。V2 不修改或删除这些表；新增迁移只增加 `v2_` 前缀表和索引。
...
6. 型号、系列、版本、条件、否定事实全部拆成单独 proposal；没有显式证据不做系列泛化，不把手册未提到转成不支持。
...
验收重点是 UX：至少 20 条真实产品知识人工喂入，覆盖明确、模糊、Telegram 碎片、冲突、型号/系列、可能、大概、否定、新旧版本和连续追问。Phase 2 测试通过后停止扩展，进入人工 UX Gate；不自动做 Phase 3。
...
- Phase 2 完成后停止在 UX Gate；Phase 3 Grounded QA、gap loop 和 Telegram V2 接入必须等人工验证结果。
